...
function Update-GitHubRelease($versionNumber, $preRelease, $releaseNotes, $rootDirectory, $gitHubUsername, $gitHubRepository, $gitHubApiUsername, $gitHubApiKey)
{
  $draft = $false
  ...
  # Upload all of the languages in the various directories
  @('syntaxes', 'generated-syntaxes') | ForEach-Object -Process {
    $Dirname = $_

    Get-ChildItem -Path (Join-Path -Path $rootDirectory -ChildPath $Dirname) | ForEach-Object -Process {
      $filename = $_.Name
      $filepath = $_.Fullname
